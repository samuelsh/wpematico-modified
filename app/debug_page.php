...
?>
	<p class="text">
		<?php _e('Use this file to get support on '); ?><a href="http://etruel.com/support/" target="_blank" rel="follow">etruel's website</a>.
	</p>
	<p></p>
	<form action="<?php echo esc_url( admin_url( 'edit.php?post_type=wpematico&page=wpematico_settings&tab=debug_info' ) ); ?>" method="post" dir="ltr">
		<label><input class="checkbox" value="1" type="checkbox" name="alsophpinfo" /> <?php _e('Include also PHPInfo() if available.', WPeMatico :: TEXTDOMAIN ); ?></label><br/>
		<label><input class="checkbox" value="1" type="checkbox" name="alsocampaignslogs" /> <?php _e('Include also Last Campaigns Log.', WPeMatico :: TEXTDOMAIN ); ?></label><br/>
		...
		<textarea readonly="readonly" id="debug-info-textarea" name="wpematico-sysinfo" 
				  title="<?php _e('To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).',  WPeMatico::TEXTDOMAIN); ?>"
				  style="width: 100%;min-height: 370px;"
		><?php 
			echo wpematico_debug_info_get(); 
		?></textarea>
			<?php  wp_nonce_field('wpematico-settings'); ?>
			<label onclick="jQuery('#debug-info-textarea').focus();jQuery('#debug-info-textarea').select()" ><?php _e('SELECT ALL', WPeMatico::TEXTDOMAIN); ?></label>
		</div>
